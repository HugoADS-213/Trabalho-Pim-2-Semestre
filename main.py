...
import csv
# Rich só pra deixar a interface mais bonitinha
...
# ==================== TODAS AS MATÉRIAS ====================
    #Abaixo são todas as materia do programa
disciplinas= ['Algoritmos e Estrutura de Dados em Python','Programação estruturada em C','Analise e Projeto de Sistemas','Engenharia de Software Agil']
...
while True:
    ...
    # Aqui começa a parte da segurança contra erro de digitação
    try:
        opcao = input("\nEscolha a disciplina: ").strip()

        # Verifica se a pessoa deixou em branco
        if opcao == "":
            print("⚠️  Você precisa digitar um número! Tente novamente.")
            continue

        # Verifica se é realmente um número
        if not opcao.isdigit():
            print("⚠️  Digite apenas números, sem letras nem símbolos!")
            continue

        # Converte pra inteiro agora que temos certeza
        opcao = int(opcao)

        # Verifica se o número está dentro das opções possíveis
        if opcao < 0 or opcao > len(disciplinas):
            print("⚠️  Opção inválida! Escolha um número entre 0 e", len(disciplinas))
            continue
    ...
    # Se chegou aqui, o valor de 'opcao' é seguro e o programa segue normal 😎
    ...
    if opcao== 0:
        ...
        for fila in filas_disciplinas.values(): #Verifica se tem algum aluno independente da materia
            if fila:
                ...
                break #Quando tem um aluno continua normalmente o código

        if not Verificar_alunos:
            print('Não foi cadastrado nenhum aluno, o arquivo não será salvo.\n'
                  'Fechando o código...')
            break

        for disciplina, fila in filas_disciplinas.items():
            if fila:  # Só salva se houver alunos cadastrados
                nome_arquivo = f"{disciplina.replace(' ', '_').replace('ã', 'a').replace('é', 'e').replace('ç', 'c')}.csv"
                ...
                with open(nome_arquivo, "w", newline="", encoding="utf-8") as arquivo:
                    ...
                    # Cabeçalho do CSV
                    escritor.writerow(["Disciplina", "Nome", "RA", "Turma", "Nota1", "Nota2", "Nota3", "Média"])
                    ...
                    for aluno in fila:
                        ...
                        while len(notas) < 3:  # Garante que existam 3 notas (ou preenche com 0)
                            notas.append(0.0)
                        #Permite que o arquive escreva linhas e já separe com vírgulas
                        escritor.writerow([
                            disciplina,
                            aluno["Nome"],
                            aluno["RA"],
        ...
        while True:
            ...
            if escolha == 1:
                while True:
                    ...
                    if confirma in ["s", "ss", "sim"]:
                        aluno = {"Nome": nome, "RA": ra, "Turma": turma, "Notas": [], "Média": 0.0}
                        fila.append(aluno)
                        print("\n✅ Aluno cadastrado com sucesso!\n"
                              "Antes de sair ao menu principal não se esqueça de colocar as notas!\n")
                    else:
                        print("\n❌ Cadastro cancelado.\n")
            ...
            # ------------------- CADASTRAR NOTAS -------------------
                ...
                while True:
                    ...
                    for i in range(1, 4):
                        while True:
                            entrada = input(f"Digite a {i}ª nota (0 a 10): ")
                            ...
                            if entrada.strip().lower() == "voltar":
                                ...
                                notas = None  # Assim a gente não grava notas incompletas
                                break
                            ...
                            try:
                                ...
                                print("Digite um número válido ou 'voltar' pra cancelar.")

                        if notas is None:
                            break  # Sai do loop de notas se o professor quiser voltar

                    # Só grava se ele realmente preencheu as 3 notas
                    if notas and len(notas) == 3:
                        aluno["Notas"] = notas
                        aluno["Média"] = round(sum(notas) / 3, 2)
                        print(f"\n✅ Média de {aluno['Nome']}: {aluno['Média']}\n"
